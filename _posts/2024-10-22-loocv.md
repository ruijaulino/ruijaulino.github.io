...
where $w_i = x_i^T\hat{b}_i$. Including transaction costs $f$ (as a percentage; of course this term can be computed as one seem fit - for example, we can compute the vector of weights and consider their successive difference and account fees only on the transacted weight), the performance adjusts to:

$s_i \propto y_i w_i - f \|w_i\|$

We made an approximation assuming the conditional variance remains relatively constant when excluding points, hence the simple proportionality. By substituting $\hat{b_i}$ we get the expression for the weights:
