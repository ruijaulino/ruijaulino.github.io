...
For the analysis, consider the eigendecomposition of the second non central moment $M_{x\|z} = U(z)\Lambda(z)U(z)^T$ and the auxiliar variable $\hat{\mu(z)} = U(z)^T\mu\_{x\|z}$ (which represents the returns of the porfolio with weights equal to the eigenvectors). Then
