...
where we sum over all strategies. Using a linear model baseline (univariate prediction of univariate returns, no biases), $x = bz + \epsilon$, $z \sim N(0, \phi^2)$, $\epsilon \sim N(0, \sigma^2)$ 
...
improving strategies can falls into more overfitting), but, nevertheless, this simple result can help into thinking about the research process. 
