...
For the analysis, consider the eigendecomposition of the second non central moment $M_{x\|z} = U(z)\Lambda(z)U(z)^T$ and the auxiliar variable $\hat{\mu(z)} = U(z)^T\mu\_{x\|z}$ (which represents the returns of the portfolio with weights equal to the eigenvectors). Then

$\text{tr}(\mathbb{E}\[ww^T\]) = \frac{1}{k^2} \sum_i \mathbb{E}\_z\[ \frac{\hat{\mu(z)}_i}{\lambda(z)_i^2} \] \approx \frac{1}{k^2} \sum_i \mathbb{E}\_z\[ \frac{\hat{\mu(z)}_i}{\lambda(z)_i} \]\mathbb{E}\_z\[ \frac{1}{\lambda(z)_i} \] = \frac{1}{k^2} g \sum_i \psi_i \mathbb{E}\_z\[ \frac{1}{\lambda(z)_i} \]$

where, using the previous definitions, $g = \sum_i \mathbb{E}\_z\[ \frac{\hat{\mu(z)}_i}{\lambda(z)_i} \]$ and $\psi_i = \frac{\mathbb{E}\_z\[ \frac{\hat{\mu(z)}_i}{\lambda(z)_i} \]}{g}$ is a weight. We can interpret this result as

$\text{tr}(\mathbb{E}\[ww^T\]) \propto \frac{1}{k^2} \frac{g}{\sigma_{\text{eff}}^2}$

as $\sigma_{\text{eff}}^2$ is a (in expectation) convex combination of variances across principal directions weighted by contribution to sharpe ($g=\sqrt{\text{SR}}$); for an almost diagonal covariance this yields a convex combination of variances.

Then, for $w$ to have order of magnitude of one, we can use as normalizer

$k = m \frac{\text{SR}}{\sigma_{\text{eff}}}$

and use the average variance as a proxy to $\sigma_{\text{eff}}$.
...
How should one proceed when we have many optimal strategies and we need to divide resources between them? Assumin low correlations between strategies, we have that the weight we give a strategy is:

$w_s = \frac{\mu_s}{\sigma_s^2} = k = m \frac{\text{SR}}{\sigma_{\text{eff}}}$
...
For example, consider a strategy $s_1$ on a single asset and a strategy $s_2$ on $p$ assets (all assets with the same volatility and all strategies with the same sharpe): we should put more money on the strategy with more assets as the convex combination yields a lower vol. 
