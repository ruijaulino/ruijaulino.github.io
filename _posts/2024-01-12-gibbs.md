...
Consider the data $D = \{x_1,x2,\cdots,x_n\}$ and that $p(x)\sim N(x\|\mu,C)$. We are interested in finding the parameters $\mu$ and $C$ that are compatible with the observations. In other words, what is $p(\mu,C\|D)$?
...
have the same functional form of the assumed prior. In other words, given that $C$ \[ $\mu$ \] is fixed, the prior for $\mu$ \[ $C$ \] is conjugate. Following the previous procedure we can apply the Gibbs sampler. 
