...
Very important to generate samples from difficult distributions; many uses in fitting models for example!
...
Consider the data $D = \{x_1,x2,\cdots,x_n\}$ and that $p(x)\sim(x\|\mu,C)$. We are interested in finding the parameters $\mu$ and $C$ that are compatible with the observations. In other words, what is $p(\mu,C\|D)$?
...
We can see that this is a semi-conjugate prior because (independence assumptions)
...
In other words, given that $C$[$\mu$] is fixed, the prior for $\mu$[$C$] is conjugate. Following the previous procedure we can apply the Gibbs sampler. 
...
Now this is more interesting because, opposite to the previous case, we cannot find a closed expession to fit the model and so this is usefull to make the fit in a more robust way.
...
For the first term we can notice that

$p(D\|\pi,\mu,C,z) = \Pi_i \left[ \Pi_j N(x_i\|\mu_j,C_j)^{I(z_i=j)} \right]$

also, $p(z_i=j\|\pi)=\pi_j$

and so we can write

$p(D\|\pi,\mu,C,z)p(z\|\pi) = \Pi_i \left[ \Pi_j \pi_j N(x_i\|\mu_j,C_j)^{I(z_i=j)} \right]$

Putting it all together and nothing that the prior for $\pi$ is a Dirichet distribution
...
The data $D$ is a sequence of observations $x_0,x_1,\cdots,x_n$.

The parameters are the transition matrix $A$, the initial state distribution $P$ and the emission parameters $\mu,C$.

In the same fashion, we can consider as variable the sequence of hidden states $z$.

Using the same reasoning, we can write the joint distribution as
...
The firt term is just the transition matrix; the second term is a familiar term from the forward pass of the HMM: it can be calculated recursivelly as:
