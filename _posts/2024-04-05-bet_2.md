...
where the second step is true because of conservation of probability (with some abuse of notation, $w$ is a function of $z$ and $p(x,w)\text{d}w\text{d}x = p(x,z)\text{d}z\text{d}x \leftarrow p(x\|w)p(w)\text{d}w = p(x\|z)p(z)\text{d}z$. 
