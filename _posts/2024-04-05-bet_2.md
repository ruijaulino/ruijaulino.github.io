...
Consider the base case where the distribution of $x$ is the same for all observations $x_i$; in this case $w$ has to be a constant vector depending of the properties of the distribution.
...
Suppose that we perturn $y(x)$ with a generic function; also we need this perturbation to be arbitrarily small. For this effect we can consider the change $y(x) \rightarrow y(x)+\epsilon \eta(x)$. The value of the function at the perturbation can be written (using expansion near $\epsilon=0$):
...
The weights at some instant $i$ must be a function of the features $z_i$, $w_i=w(z_i)$. The growth rate becomes
...
where the second step is true because of conservation of probability (with some abuse of notation, $w$ is a function of $z$ and $p(z)\text{d}z=p(w)\text{d}w$. $p(x,z)$ was written as $p(x\|z)p(z)$.


Optimization of $G$ is a similar problem to the one of calculus of variations. To find the functional form of the weights that maximize $G$ we can resort to the prevous formula from calculus of variations:
...
we can note that, $\text{SR}(w)=\text{SR}(kw)$, i.e, multiplication by a constant does not change the result. Based on this we can consider the maximization constrained to the denominator equal to unit. Using Lagrange multpliers we can consider maximizing the following functional as the same as optimizing sharpe:
...
With this new notation, let us perturb $w(z)$ with $w(z)+\epsilon \eta(z)$:
...
$\int xp(x\|z) -\lambda xx^T w  \text{d}x = 0$

Solving yields (similar to maximization of growth rate)

$w=\lambda^{-1} \Sigma_{x\|z}^{-1}\mu_{x\|z}$
