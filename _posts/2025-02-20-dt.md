...
$w^* = \left\[ \int yy^T p(y\|x) \text{d}y \right]^{-1} \left\[ \int y p(y\|x) \text{d}y \right\] = M_{y\|x}^{-1}\mu\_{y\|x}$

where $M\_{y\|x}$ is conditional second non central moment and $\mu\_{y\|x}$ the conditional expected value.
