...
This setup is used many times: we define a model (or rule if we want) to bet on a trade based on some information and we try to _optimize_ it generate more money.


#### Example

Let $y,x$ be scalars and assume a linear model for the weight $w = ax$ (this idea just states that what we should be is proportional to the magnitude of the feature; of course, there are simpler examples, like deciding just to go long or short with the available money). Given a set of examples, maximizing $G$ yields the estimator:
