...
$w^* = \left[ \int yy^T p(y|x) \text{d}y \right]^{-1} \left[ \int y p(y|x) \text{d}y \right] = M_{y|x}^{-1}\mu_{y|x}$

where $M_{y|x}$ is conditional second non central moment and $\mu_{y|x}$ the conditional expected value.
