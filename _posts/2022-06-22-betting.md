...
One way to look at this is to note that $\mu$ has dimensions of _percentage per unit of time_ (percentage changes do not have dimension of course but they are measured on some time scale - the one used to compute $\frac{p(t+\tau)-p(t)}{p(t)}$) and $\sigma^2$ has 
