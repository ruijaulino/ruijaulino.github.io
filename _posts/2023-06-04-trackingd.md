...
This is similar to a Kalman filter (the state is the mean) with heteroscedastic noise. Given a new return observation, the objective is to compute the filtering distribution:
...
And so, at any given instant, the previous observations are weighted as $\phi^t(1-\phi)=\phi^t-\phi^{t+1}$ for the mean and $\phi^{t+1}$ for the covariance ($t$ means the relative distance to the current time step). In both cases this represents a exponential decay in importance of datapoints for the computation of the moments.
