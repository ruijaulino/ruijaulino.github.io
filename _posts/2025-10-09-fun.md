...
from where we can see we have two contributions: one from how the weights are correlated with the targets (note that weights must have positive correlation with targets - but not the features, of course - otherwise it does not make sense to use them) and another from the expected values. The analogue to the no edge case is $\mathbb{E}\left\[x\right] = 0$ and $\text{cov}\left(w(z), x\right) = 0$ for any $x$.
